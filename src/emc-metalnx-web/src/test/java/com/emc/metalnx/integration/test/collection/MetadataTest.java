...
public class MetadataTest {
    ...
    /**
     * Test method for adding a new metadata tag to a collection or file.
     * It verifies if this new metadata was added to the list and if metadata count was incremented
     * by one
     * */
    @Test
    ...
    /**
     * Test method for modifying a metadata tag from a collection or file.
     * It verifies if this new metadata was modified
     * */
    @Test
    public void testModMetadata() {
        ...
        // adding a new metadata in case there is no metadata attached in the colletcion/file
        ...
        wait.until(
                ExpectedConditions.visibilityOfElementLocated(By.cssSelector("#metadaTable tbody tr:first-child button.enableEditAVUActionColumn")))
                .click();
        // driver.findElement(By.cssSelector("#metadaTable tbody tr:first-child button.enableEditAVUActionColumn")).click();

        // inputs of the form
        wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("#metadaTable tbody tr:first-child .saveEditAVUActionColumn")));
    ...
    /**
     * Test method for deleting a metadata tag from a collection or file.
     * It verifies if this new metadata was deleted from the list
     * */
    @Test
    public void testDelMetadata() {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("#metadaTable tbody tr:first-child .metadataCheckbox")));
    ...
    /**
     * Test method to try creating an existing metadata tag from a collection or file.
     * It verifies if this metadata was not duplicated
     * */
    @Test
    public void testSaveExistingMetadata() {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".browse-collection-files-content .alert-danger")));
    ...
    /**
     * Test method to try creating an empty metadata tag in a collection or file.
     * It verifies if this metadata was not created
     * */
    @Test
    ...
    /**
     * Test method to try creating a large metadata tag in a collection or file.
     * It verifies if this metadata was created without problems
     * Limit for metadata value is 64 characters
     * */
    @Test
    ...
    /**
     * Test method for deleting all metadata from a collection or file.
     * It verifies if all metadata from a collection/file were deleted
     * */
    @Test
    @Ignore
    ...
    /**
     * Method to test if a certain WebElement contains the avu tuple passed as parameters
     * */
    private boolean avuExistsInWebElement(WebElement we, String attr, String val, String unit) {
        if (attr.equals(we.getAttribute("data-attr")) && val.equals(we.getAttribute("data-val")) && unit.equals(we.getAttribute("data-unit"))) {
            return true;
        }
        return false;
    }
    ...
    private void filterTableAndWaitResult(String searchString) {
        wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("#metadaTable_length select[name='metadaTable_length']")));
        Select selectPageSize = new Select(driver.findElement(By.cssSelector("#metadaTable_length select[name='metadaTable_length']")));
        selectPageSize.selectByValue("100");
        wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("#metadaTable_filter input"))).sendKeys(searchString);
