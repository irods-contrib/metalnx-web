...
public class ApplyTemplateOnFilesTest {
    ...
    /**
     * Method that tests if a template can be applied to a file. The template is applied as it is.
     */
    @Test
    ...
    /**
     * Method that tests if a template can be applied to a file. The template values are changed.
     */
    @Test
    public void testApplyTemplateChangingValues() {
        ...
        TemplateUtils.submitApplyTemplateForm(driver);
        TemplateUtils.isSuccessMessageShown(driver);
        TemplateUtils.checkIfMetadataWasAdded(driver, false, attributes, changedValues, units, TemplateUtils.TEST_FILES);
    }

    /**
     * Auxiliar method that changes the attribute values of a template just before it is applied to
     * a file. It assumes the template was already found previously and the driver is on the right
     * page.
     */
