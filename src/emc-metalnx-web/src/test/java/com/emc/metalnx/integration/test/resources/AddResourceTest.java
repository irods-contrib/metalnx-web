...
public class AddResourceTest {
    ...
    /**
     * Test method for adding a brand new resource into the system. It verifies after adding
     * a resource if the user is redirected to the resource management page and a successful
    ...
    public void testAddUnixFileSystemResource() {
        ...
        ResourceUtils.addResource(UITest.RESOURCES_URL, RESOURCE_NAME, ResourceUtils.RESC_UNIX_FILE_SYSTEM, null, null, UITest.IRODS_HOST,
                ResourceUtils.RESOURCE_PATH, driver);
    }

    /**
     * Test method for adding a resource with same name of another already created. It verifies
     * if it remains in same page and does not add new resource.
     *
     * @throws Exception
     */
    @Test
    public void testAddSameResourceName() throws Exception {
        ...
        ResourceUtils.fillAndSubmitResourceForm(RESOURCE_NAME, ResourceUtils.RESC_COMPOUND, null, null, null, null, driver);
    ...
    /**
     * Test method for adding a new resource in resource map. It adds parent resource through management page, adds its
     * resource child through map page and verifies with both are displayed.
     */
    @Test
    @Ignore
    // TODO Test ignored when executing HTMLUnit driver. There is no support for d3.select() function in SVG element.
    public void testAddResourceFromResourceMap() {
        logger.info("Testing add a brand new resource from resource map");
        ResourceUtils.addResource(UITest.RESOURCES_URL, RESOURCE_NAME, ResourceUtils.RESC_COMPOUND, null, null, null, null, driver);
        ResourceUtils
                .addResource(UITest.RESOURCES_MAP_URL, CHILD_RESOURCE_NAME, ResourceUtils.RESC_COMPOUND, RESOURCE_NAME, null, null, null, driver);
    ...
    /**
     * Test method for adding a new resource in servers view. It verifies if resource is displayed in servers view after
     * adding.
     */
    @Test
    public void testAddResourceFromServersView() {
        ...
        ResourceUtils.addResource(UITest.RESOURCES_SERVERS_URL, RESOURCE_NAME, ResourceUtils.RESC_DEFERRED, null, null, UITest.IRODS_HOST,
                ResourceUtils.RESOURCE_PATH, driver);

        assertTrue(ResourceUtils.isInResourcesList(UITest.RESOURCES_SERVERS_URL, RESOURCE_NAME, UITest.IRODS_HOST, driver));
    }

    /**
     * Test method for trying to add a brand new resource into the system with resource name empty.
     * It verifies if system does not add it and throw a error message.
     */
    @Test
    ...
    /**
     * Test method for adding a resource with a invalid name, with blank spaces. It verifies
     * if it remains in same page and does not add new resource.
     */
    @Test
    ...
    /**
     * Test method for adding a storage resource with empty host. It verifies if it remains in
     * same page, displays error message and does not add new resource.
     */
    @Test
    public void testAddStorageResourceWithEmptyHost() {
        logger.info("Testing add resource with empty host");
        ResourceUtils.accessAddResourceFormFrom(UITest.RESOURCES_URL, null, driver);
        ResourceUtils.fillAndSubmitResourceForm(RESOURCE_NAME, ResourceUtils.RESC_UNIX_FILE_SYSTEM, null, null, "", ResourceUtils.RESOURCE_PATH,
                driver);
    ...
    /**
     * Test method for adding a storage resource with empty path. It verifies if it remains in
     * same page, displays error message and does not add new resource.
     */
    @Test
    public void testAddStorageResourceWithEmptyPath() {
        ...
        ResourceUtils.accessAddResourceFormFrom(UITest.RESOURCES_URL, null, driver);
        ResourceUtils.fillAndSubmitResourceForm(RESOURCE_NAME, ResourceUtils.RESC_UNIX_FILE_SYSTEM, null, null, UITest.IRODS_HOST, "", driver);
    ...
    /**
     * Test method for canceling add resource. It verifies if it redirects to Resources Management
     * page and does not add new resource.
     */
    @Test
    ...
    /**
     * Test method for not canceling add resource. It verifies if it remains in add resource
     * form page.
     */
    @Test
