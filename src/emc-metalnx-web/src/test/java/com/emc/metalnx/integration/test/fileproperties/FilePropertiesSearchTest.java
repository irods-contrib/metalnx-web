...
public class FilePropertiesSearchTest {
    ...
    public void searchNotEqualsOneCriteriaWithResults() {
        ...
        fillSearchInFisrtField(AttributeOptions.FILE_PATH.toString(), OperatorOptions.NOT_EQUAL.toString(), userHomeColl);
    ...
    public void searchNotContainsOneCriteriaWithResults() {
        ...
        fillSearchInFisrtField(AttributeOptions.FILE_PATH.toString(), OperatorOptions.NOT_LIKE.toString(), userHomeColl);
    ...
    public void searchMultipleCriteriasWithResults() {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".propertiesSearchRow:nth-of-type(2)")));
        ...
        // adding a new criteria search field
        driver.findElement(addSearchParamLocator).click();
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".propertiesSearchRow:nth-of-type(3)")));
    ...
    /**
     * Creates a new user to test if file properties search can find objects that rodsadmin user has no permission on
     */
    @Test
    public void searchForObjsWithoutPermission() {
        ...
        // if there is at least one <a> tag, then it means there are objects which the current user has access to
        wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector("#treeViewTable tbody tr")));
        Assert.assertTrue(driver.findElements(By.cssSelector("#treeViewTable tbody > tr > td:first-child > span")).size() != 0);
    ...
    private void submitAndCheckNotEmptyTable() {
        // click submit
        driver.findElement(submitPropertiesSearchLocator).click();
        // if there is at least one <a> tag, then it means there are objects which the curretn user has access to
        wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector("#treeViewTable tbody tr")));
        Assert.assertTrue(driver.findElements(By.cssSelector("#treeViewTable tbody a")).size() != 0);
    }

    /**
     * Compares path column to check if value from the table is the same as the one expected to be
     */
    private void checkIfUploadedFileExist() {
        Assert.assertEquals(driver.findElements(By.cssSelector("#treeViewTable tbody a")).size(), 1);
        Assert.assertEquals(filePath, driver.findElement(By.cssSelector("#treeViewTable tbody tr td:nth-child(2)")).getText());
    }
     ...
     * @param value
     * @param nth
     *            represents the nth search parameter in which the options above are to be placed and it starts at 1
     */
    private void fillSearchInNthField(String attribute, String operator, String value, Integer nth) {
        new Select(driver.findElement(By.cssSelector(".propertiesSearchRow:nth-of-type(" + nth.toString() + ") .propertiesAttr")))
                .selectByValue(attribute);
        new Select(driver.findElement(By.cssSelector(".propertiesSearchRow:nth-of-type(" + nth.toString() + ") .propertiesOper")))
                .selectByValue(operator);
        driver.findElement(By.cssSelector(".propertiesSearchRow:nth-of-type(" + nth.toString() + ") .propertiesVal")).sendKeys(value);
    }
}
