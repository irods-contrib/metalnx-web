...
public class ViewInfoResourceTest {
    ...
    private static String RESOURCE_PARENT = UITest.IRODS_ZONE;
    private static String RESOURCE_ZONE = UITest.IRODS_ZONE;
    private static String RESOURCE_HOST = UITest.IRODS_HOST;
    ...
    public void setUp() throws Exception {
        ...
        ResourceUtils.addResource(UITest.RESOURCES_MAP_URL, RESOURCE_COORDINATING_NAME, RESOURCE_COORDINATING_TYPE, RESOURCE_PARENT, RESOURCE_ZONE, RESOURCE_HOST, RESOURCE_PATH, driver);
        ResourceUtils.addResource(UITest.RESOURCES_MAP_URL, RESOURCE_STORAGE_NAME, RESOURCE_STORAGE_TYPE, RESOURCE_PARENT, RESOURCE_ZONE, RESOURCE_HOST, RESOURCE_PATH, driver);
    }
    ...
    /**
     * Test method for view info in resource management page. It verifies if information displayed in info modal
     * matches with information added for a coordinating resource.
     */
    @Test
    @Ignore
    ...
    public void testViewInfoCoordinatingRescInResourceManagement(){
    	...
    	assertEquals(UITest.RESOURCES_URL, driver.getCurrentUrl());
    	
    	if(ResourceUtils.isInResourcesList(UITest.RESOURCES_URL, RESOURCE_COORDINATING_NAME, null, driver)){
    		...
    		ResourceUtils.assertResourceInfo(RESOURCE_COORDINATING_NAME, RESOURCE_COORDINATING_TYPE, RESOURCE_ZONE, RESOURCE_ZONE, RESOURCE_PATH, RESOURCE_PARENT, driver);
    ...
    /**
     * Test method for view info in resource map page. It verifies if information displayed in info modal
     * matches with information added for a coordinating resource.
     */
    @Test
    @Ignore
    ...
    public void testViewInfoCoordinatingRescInResourceMap(){
    	...
    	driver.get(UITest.RESOURCES_MAP_URL);
    	WebDriverWait wait = new WebDriverWait(driver, 8);
    	wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.id("resourceMapPanel")));
    	assertEquals(UITest.RESOURCES_MAP_URL, driver.getCurrentUrl());
    ...
    /**
     * Test method for view info in resource management page. It verifies if information displayed in info modal
     * matches with information added for a storage resource.
     */
    @Test
    @Ignore
    // TODO Test ignored when executing HTMLUnit driver. There is no support for d3.select() function in SVG element.
    public void testViewInfoStorageRescInResourceManagement(){
    	...
    	if(ResourceUtils.isInResourcesList(UITest.RESOURCES_URL, RESOURCE_STORAGE_NAME, null, driver)){
    		driver.findElement(By.id("btn_info_" + RESOURCE_STORAGE_NAME)).click();
    		
    		By rescInfoModal = By.id("rescInfoModal");
    		wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(rescInfoModal));
    		
    		ResourceUtils.assertResourceInfo(RESOURCE_STORAGE_NAME, RESOURCE_STORAGE_TYPE, RESOURCE_ZONE, RESOURCE_HOST, RESOURCE_PATH, RESOURCE_PARENT, driver);
