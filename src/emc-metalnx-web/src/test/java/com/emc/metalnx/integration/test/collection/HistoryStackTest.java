...
public class HistoryStackTest {
    ...
    private static String[] arrayPaths = { CollectionUtils.RODS_COLL_PATH, String.format("%s/%s", CollectionUtils.RODS_COLL_PATH, collectionName1),
            String.format("%s/%s/%s", CollectionUtils.RODS_COLL_PATH, collectionName1, collectionName2),
            String.format("%s/%s/%s/%s", CollectionUtils.RODS_COLL_PATH, collectionName1, collectionName2, collectionName3) };
    ...
    private By inactivePathHistory = By.cssSelector("#directoryPath > .btn-group > ul.dropdown-menu > li:not(.active) a");
    ...
    public static void tearDownAfterClass() {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("#directoryPath .breadcrumb > li:last-of-type > span")));
    ...
    /**
     * Test if stack to go back in collection history is working and putting paths in the correct
     * order. Creates collection and enters them to fill stack
     */
    @Test
    ...
    /**
     * Test if both back and forward stacks are in order if more than 1 step back is taken
     */
    @Test
    ...
    /**
     * Test if back and forward stacks are in order is more than 1 step forward is taken
     */
    @Test
    ...
    private boolean checkCurrentPath(WebDriver driver, WebDriverWait wait, String expectedPath) {
        ...
        String currentPathInStack = driver.findElement(By.cssSelector("#directoryPath > .btn-group > .dropdown-menu > .active a")).getText();
    ...
    private void clickNthChildInHistory(WebDriver driver, WebDriverWait wait, int nth) {
        String selectorString = String.format("#directoryPath > .btn-group > ul.dropdown-menu > li:nth-child(%d) a", nth);
        wait.until(ExpectedConditions.elementToBeClickable(dropdownHistoryButtonLocator)).click();
        wait.until(ExpectedConditions.visibilityOfElementLocated(dropdownHistoryMenuLocator));
