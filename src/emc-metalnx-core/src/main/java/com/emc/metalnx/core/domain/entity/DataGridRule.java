...
public class DataGridRule {
	...
	public void setInputRuleParams(String... params) {
		this.inputRuleParams = params;
	}

	public void setOutputRuleParams(String... params) {
		this.outputRuleParams = params;
	}
	...
	private String getInputParamsAsString() {
		...
		if (inputRuleParams != null) {
			...
			for (int i = 0; i < params.length - 1; i++)
				sb.append(String.format(" *p%d=\"%s\",", i, params[i]));
	...
	private String getMSIParamsAsString() {
		...
		if (inputRuleParams != null) {
			for (int i = 0; i < inputRuleParams.length - 1; i++)
				paramsEscaped.append(String.format("*p%d, ", i));
		...
		if (outputRuleParams != null) {
			for (int i = 0; i < outputRuleParams.length - 1; i++)
				paramsEscaped.append(String.format("*%s, ", outputRuleParams[i]));
	...
	private String initializeOutputParams() {
		...
		for (int i = 0; i < outputRuleParams.length - 1; i++)
			outputParams.append(String.format("*%s=\"\",", outputRuleParams[i]));
